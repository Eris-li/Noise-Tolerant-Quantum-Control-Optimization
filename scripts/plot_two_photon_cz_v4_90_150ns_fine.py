...
from neutral_yb.config.artifact_paths import v4_fine_90_150ns_10mhz_dir
...
def main() -> None:
    artifacts = v4_fine_90_150ns_10mhz_dir(ROOT)
    summary = json.loads(
        (artifacts / "two_photon_cz_v4_90_150ns_0p5ns_10mhz_fine_summary.json").read_text(encoding="utf-8")
    )
    points = summary["points"]
    best = max(points, key=lambda point: float(point["probe_fidelity"]))
    threshold = 0.999
    first_threshold = summary.get("first_threshold_point")
    ...
    ax = axes[0, 0]
    ax.plot(gate_times, fidelities, marker="o", linewidth=2.0, markersize=4.5, color="#1f78b4")
    ax.axhline(threshold, color="#d95f0e", linestyle="--", label="F = 0.999")
    if first_threshold is not None:
        ax.axvline(float(first_threshold), color="#6a3d9a", linestyle=":", label=f"first F≥0.999: T={float(first_threshold):.1f} ns")
    ax.set_xlabel("UV Segment Time (ns)")
    ax.set_ylabel("Phase-Gate Fidelity")
    ax.set_ylim(0.80, 1.0005)
    ax.set_title("v4 10 MHz fine scan (90-150 ns)")
    ax.legend()
    ...
    ax.set_title("Optimized amplitude and phase at fine optimum")
    ...
    clock_indices = [index for index, label in enumerate(basis_labels) if label in {"0c", "W_c", "cc"}]
    ...
    rydberg_indices = [index for index, label in enumerate(basis_labels) if label in {"0r", "W_r"}]
    ...
    fig.suptitle("^171Yb v4 full-gate fine scan at 10 MHz")
    fig.tight_layout()

    output_path = artifacts / "two_photon_cz_v4_90_150ns_0p5ns_10mhz_fine_summary.png"
    ...
    pulse_fig.suptitle("^171Yb v4 optimized UV control at fine optimum")
    pulse_fig.tight_layout()
    pulse_output_path = artifacts / "two_photon_cz_v4_90_150ns_0p5ns_10mhz_fine_optimized_pulse.png"
