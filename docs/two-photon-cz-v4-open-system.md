...
#### `clock_pulse_area_fractional_rms`

源码默认：

- `clock_pulse_area_fractional_rms = 0.002`

采样方式：

```python
clock_amplitude_scale ~ Normal(1.0, 0.002)
```

含义：

- 这表示前后两段固定 `clock shelving / unshelving` 复合脉冲的**整体幅度标定误差**
- 对每个 shot，前后缀 `clock` 脉冲的振幅整体乘上一个常数因子
- 这个因子在同一个 shot 内不随时间变化

物理上它对应：

- `clock` 驱动功率的 shot-to-shot 波动
- `clock` 脉冲面积标定误差
- 慢漂移而不是快噪声
...
- `quasistatic_clock_detuning_rms_hz = 250.0`

采样方式：

```python
common_clock_detuning ~ Normal(0, 250 Hz)
```

在 `10 MHz` 参考频率下，对应无量纲量级：

```math
250 / 10^7 = 2.5 \times 10^{-5}
```
...
| 参数 | 当前默认值 | 代码字段 | 用法 | 主要来源 |
| --- | ---: | --- | --- | --- |
| Clock 幅度 rms 误差 | `0.002` | `clock_pulse_area_fractional_rms` | 进入 quasistatic `clock_amplitude_scale` | 当前 `^171Yb` 默认保守非零 |
| UV 幅度 rms 误差 | `0.004` | `uv_pulse_area_fractional_rms` | 进入 quasistatic `uv_amplitude_scale` | 与文献量级一致的当前默认 |
| Clock 准静态共模失谐 rms | `250 Hz` | `quasistatic_clock_detuning_rms_hz` | ensemble 采样 | 当前 `^171Yb` 默认保守非零 |
