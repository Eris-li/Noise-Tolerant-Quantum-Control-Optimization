...
因此，当前默认实验扩展 profile 里，dephasing 已经显式加进去了，但只加了 `Rydberg` 的快纯退相干；`clock` 的 Markovian dephasing 仍然默认关闭。
...
- `build_yb171_v4_calibrated_model(profile=...)`
- `build_yb171_v4_quasistatic_ensemble(profile=...)`
...
如果把前面所有定义压缩成一组统一公式，那么当前 `v4` 模型可以概括成下面这套方程。
